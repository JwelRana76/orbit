...
class MedicinePurchaseService {
    ...
    function index()
    {
        ...
        ->addColumn('action', fn ($item) => view('pages.medicine_purchase.action', compact('item'))->render())
    ...
    function store($data)
    {
        ...
        $purchase_data['medicine_supplier_id'] = $data['supplier'];
        ...
        foreach ($data['medicine_id'] as $key => $item) {
            $medicine_purchase = [
                ...
                'medicine_id'          => $item,
                'qty'              => $data['qty'][$key],
                'bonus'              => $data['bonus'][$key],
                'price'            => $data['cost'][$key],
            ];

            MedicinePurchaseItem::create($medicine_purchase);
        }
    ...
    function update($data,$id)
    {
        ...
        $payment['medicine_purchase_id'] = $purchase->id;
        $payment['paid_amount'] = $data['paid'];
        $this->payment($payment);
        foreach ($data['medicine_id'] as $key => $item) {
            $medicine_purchase = [
    ...
    public function payment($data){
        if($data["paid_amount"] > 0){
            $payment['medicine_purchase_id'] = $data["id"];
            $payment['amount'] = $data["paid_amount"];
            $payment['user_id'] = Auth::user()->id;
            MedicinePayment::create($payment);
        }
    }
}