<x-admin title="Customer">
    <x-page-header head="Customer" />
    <div class="row">
        <div class="col-md-3">
            <div class="card p-3">
                <x-form method="post" action="{{ route('medicine.customer.store') }}">
                    <x-input id="id" type="hidden" value="{{ $data->id ?? null }}" />
                    <x-input id="name" required value="{{ $data->name ?? old('name') }}" />
        ...
        <div class="col-md-9">
            <x-data-table dataUrl="pharmacy/medicine/customer" id="medicinecustomers" :columns="$columns" />
        </div>
    </div>
    ...
    <div class="modal fade" id="paymentModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-body">
                <x-form method="post" action="{{ route('medicine.customer.payment') }}">
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#paymentDetails').on('show.bs.modal', function (event) {
                    ...
                    $.get("/pharmacy/medicine/customer/payment/details/" + id, function (data) {
                        ...
                        $.each(data, function(index, item) {
                            tbody.append(`
                                <tr>
                                    ...
                                    <td>${item.amount}</td>
                                    <td>
                                        <a onclick="return confirm('Are you sure to Delete this record..??')" href="{{ url('pharmacy/medicine/sale/paymentdelete') }}/${item.id}">
